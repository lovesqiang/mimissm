...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            商品名称：<input name="shopName" id="shopName">&nbsp;&nbsp;&nbsp;
            商品类型：<select name="shopTypeid" id="shopTypeid">
            <option value="-1">请选择</option>
            <c:forEach items="${ptlist}" var="pt">
                <option value="${pt.shopTypeid}">${pt.typeName}</option>
            </c:forEach>
        </select>&nbsp;&nbsp;&nbsp;
                ...
                <div id="middle">
                    <table class="table table-bordered table-striped">
                        ...
                        <c:forEach items="${info.list}" var="p">
                            <tr>
                                <td valign="center" align="center"><input type="checkbox" name="ck" id="ck" value="${p.shopId}" onclick="ckClick()"></td>
                                <td>${p.shopName}</td>
                                <td>${p.shopIntroduce}</td>
                                <td>${p.shopPrice}</td>
                                <td><img width="55px" height="45px"
                                         src="${pageContext.request.contextPath}/${p.shopImage}"></td>
                                <td>${p.shopCount}</td>
                                    <%--<td><a href="${pageContext.request.contextPath}/admin/product?flag=delete&shopId=${p.shopId}" onclick="return confirm('确定删除吗？')">删除</a>--%>
                                    <%--&nbsp;&nbsp;&nbsp;<a href="${pageContext.request.contextPath}/admin/product?flag=one&shopId=${p.shopId}">修改</a></td>--%>
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="one(${p.shopId},${info.pageNum})">编辑
                                    </button>
                                    <button type="button" class="btn btn-warning" id="mydel"
                                            onclick="del(${p.shopId})">删除
                                    </button>
                                </td>
                            </tr>
                        </c:forEach>
...
alert(str+"11111111");
    ...
    function del(shopId) {
        if (confirm("确定删除吗")) {
          //向服务器提交请求完成删除
            window.location="${pageContext.request.contextPath}/prod/delete.action?shopId="+shopId;
        }
    }

    function one(shopId, ispage) {
        location.href = "${pageContext.request.contextPath}/prod/one.action?shopId=" + shopId + "&page=" + ispage;
    }
</script>
